...
function CornerMarkers({ color }: { color: string }) {
  ...
  return (
    <>
      <div
        className={`${cls} top-0 left-0 border-t border-l`}
        style={{ borderColor: color }}
      />
      <div
        className={`${cls} top-0 right-0 border-t border-r`}
        style={{ borderColor: color }}
      />
      <div
        className={`${cls} bottom-0 left-0 border-b border-l`}
        style={{ borderColor: color }}
      />
      <div
        className={`${cls} bottom-0 right-0 border-b border-r`}
        style={{ borderColor: color }}
      />
    </>
  );
      ...
      >
        <div className="mx-auto w-full max-w-7xl px-6">
          ...
          <div className="mb-8 flex flex-col items-center text-center">
            <div className="border-gradient-spin inline-flex items-center gap-2 bg-black/60 px-4 py-1.5 mb-2">
            ...
            <h2 className="font-manrope mb-3 text-3xl font-bold leading-tight text-white md:text-4xl lg:text-5xl">
              About the <span style={{ color: accent }}>Project</span>
            </h2>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:gap-10">
              ...
              <ul className="flex flex-col gap-2">
                {PRODUCT_HIGHLIGHTS.map((highlight, i) => (
                  <li key={i} className="flex items-start gap-2">
                    <span
                      className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center text-xs"
                      style={{ color: accent }}
                    >
                      ✓
                    </span>
                    <span className="font-inter text-sm leading-5 text-zinc-100">
                      {highlight.text}
                    </span>
                  </li>
                ))}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                <h3
                  className="font-mono mb-3 text-[10px] font-bold uppercase tracking-widest"
                  style={{ color: accent }}
                >
                  [ Tech Stack ]
                </h3>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {TECH_STACK_CARDS.map((card) => (
                    ...
                    >
                      ...
                      <span className="font-inter text-[10px] leading-4 text-white/50">
                        {card.description}
                      </span>
                    </div>
                  ))}
                ...
                <div className="mb-3 grid grid-cols-4 gap-3">
                  {codestats.map((stat) => (
                    <div key={stat.label} className="flex flex-col">
                      <span className="font-manrope text-3xl font-medium text-white tracking-tighter">
                        {stat.value}
                      </span>
                      <span className="font-inter text-[10px] text-zinc-400">
                        {stat.label}
                      </span>
                    </div>
                  ))}
    ...
    >
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mb-14 flex flex-col items-center text-center">
          <span className="mb-6 inline-block bg-black border border-zinc-800 border-dashed rounded-none px-4 py-1.5 font-mono text-[10px] uppercase tracking-widest text-[#007bff] font-bold">
            [ Smart Pen Platform ]
          </span>
          <h2 className="font-manrope mb-4 text-4xl font-bold leading-tight text-white md:text-5xl">
            About the <span className="text-[#007bff]">Project</span>
          </h2>
        ...
        <div className="grid gap-12 md:grid-cols-2 lg:gap-16">
          ...
          <div className="flex flex-col gap-8">
            <div>
              <h3 className="font-mono mb-4 text-[10px] font-bold uppercase tracking-widest text-[#007bff]">
                [ Tech Stack ]
              </h3>
            ...
            <div className="relative bg-black border border-zinc-800 border-dashed rounded-none p-6">
              <CornerMarkers color="#007bff" />
              <p className="font-mono mb-3 text-[10px] font-bold uppercase tracking-widest text-[#007bff]">
                [ Codebase ]
              </p>
